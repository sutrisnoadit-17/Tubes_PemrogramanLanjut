...
import time
from config.api import testConnection

def msgbox():
    messagebox.showwarning("Connection Error","Cek Koneksimu Apakah Terhubung Dengan Internet")
    return None
...
def loadingScreen():
    ...
    label1=Label(w, text='   Jadwal Sholat', fg='white', bg='#272727') 
    ...
    image_a=ImageTk.PhotoImage(Image.open('assets/l2.png'))
    image_b=ImageTk.PhotoImage(Image.open('assets/l1.png'))

    for i in range(5): 
        if i>2:
            label2=Label(w, text='Check Your Connection...', fg='white', bg='#272727') 
            label2.configure(font=("Calibri", 11))
            label2.place(x=10,y=215)
